...
# ========== АВТО-СООБЩЕНИЯ (после 15-20 сообщений, но не чаще 3 часов) ==========
chat_msg_count = {}
chat_last_auto = {}
...
async def check_auto_message(chat_id: int):
    ...
    # Проверяем, прошло ли 3 часа с последнего авто-сообщения
    if chat_id in chat_last_auto:
        time_diff = now - chat_last_auto[chat_id]
        if time_diff.total_seconds() < 10800:  # 3 часа
            return
    
    # Проверяем, накопилось ли 15-20 сообщений
...
async def catch_all(message: types.Message):
    if message.chat.type in ["group", "supergroup"]:
        ...
        # Считаем сообщения для авто-ответа (только не от бота)
        if message.from_user.id != bot.id:
            chat_id = message.chat.id
            chat_msg_count[chat_id] = chat_msg_count.get(chat_id, 0) + 1
            await check_auto_message(chat_id)

# ========== КОМАНДЫ БЕЗ СЛЕША (все на русском) ==========

# КУБЫ
@dp.message()
async def handle_cubes(message: types.Message):
    if not message.text:
        return
...
@dp.message()
async def handle_duel(message: types.Message):
    if not message.text:
        return
    text = message.text.lower().strip()
...
@dp.message()
async def handle_knb(message: types.Message):
    if not message.text:
        return
    text = message.text.lower().strip()
...
# ========== КОМАНДЫ БЕЗ СЛЕША (МОДЕРАЦИЯ И СТАТИСТИКА) ==========
...
async def handle_moderation_commands(message: types.Message):
    ...
    # МОЙ УРОВЕНЬ
    if text == "моя роль" or text == "мой уровень":
        level = await get_role(message.from_user.id, message.chat.id)
        await message.reply(f"🎭 Ваш РП-уровень: **{level}** из 10", parse_mode=ParseMode.MARKDOWN)
        return
    
    # АДМИНЫ
    if text == "админы" or text == "рп админы" or text == "список админов":
        conn = sqlite3.connect("fable_bot.db")
        c = conn.cursor()
    ...
    # СТАТИСТИКА
    if text == "стат" or text == "статистика":
        conn = sqlite3.connect("fable_bot.db")
        c = conn.cursor()
