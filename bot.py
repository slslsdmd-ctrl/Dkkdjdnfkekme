...
async def handle_all_messages(message: types.Message):
    ...
    if message.new_chat_members:
        for user in message.new_chat_members:
            if user.id == bot.id:
                # Бота только что добавили — даём 10 уровень ВСЕМ АДМИНАМ
                try:
                    ...
                    if admin_list:
                        await bot.send_message(
                            message.chat.id,
                            f"👑 Всем администраторам группы выдан 10 уровень!\n\n📋 Список: {', '.join(admin_list)}"
                        )
                except Exception as e:
                    print(f"Ошибка при выдаче уровня админам: {e}")
                continue
            
            # ТВОЙ АЙДИ — ВСЕГДА 10 УРОВЕНЬ
    ...
    if text_lower == "кубы":
        if message.reply_to_message:
            ...
            msg1 = await bot.send_dice(message.chat.id, emoji="🎲")
            await asyncio.sleep(0.5)
            msg2 = await bot.send_dice(message.chat.id, emoji="🎲")
            await asyncio.sleep(0.5)
            msg3 = await bot.send_dice(message.chat.id, emoji="🎲")
            await asyncio.sleep(0.5)
            msg4 = await bot.send_dice(message.chat.id, emoji="🎲")
            
            total1 = msg1.dice.value + msg2.dice.value
            total2 = msg3.dice.value + msg4.dice.value
            
            if total1 > total2:
                result_text = f"🏆 Победил {await get_name(challenger_id)}! ({total1} vs {total2})"
            elif total2 > total1:
                result_text = f"🏆 Победил {await get_name(opponent_id)}! ({total1} vs {total2})"
            else:
                result_text = f"🤝 Ничья! ({total1} vs {total2})"
            
            await message.reply(
                f"🎲 **Результат:**\n\n"
                f"{await get_name(challenger_id)}: {msg1.dice.value} + {msg2.dice.value} = {total1}\n"
                f"{await get_name(opponent_id)}: {msg3.dice.value} + {msg4.dice.value} = {total2}\n\n"
                f"{result_text}",
            ...
            await message.reply(
                f"🎲 {message.from_user.first_name} кидает кости...\n"
                f"{msg1.dice.value} + {msg2.dice.value} = **{total}**",
                parse_mode=ParseMode.MARKDOWN
            )
        return
    ...
    # ОРЁЛ/РЕШКА
    if text_lower in ["орёл", "орел", "решка"]:
        if not message.reply_to_message:
            await message.reply("ℹ️ Чтобы сыграть в Орёл/Решку, ответь на сообщение противника и напиши **Орёл** или **Решка**", parse_mode=ParseMode.MARKDOWN)
            return
        ...
        user_choice = "орёл" if text_lower in ["орёл", "орел"] else "решка"
        ...
        await message.reply(f"🪙 **{await get_name(challenger_id)}** вызывает **{await get_name(opponent_id)}** на **Орёл/Решку**!\n\n"
                           ...
                           f"👇 {await get_name(opponent_id)}, нажми на кнопку, чтобы кинуть монетку:",
                           parse_mode=ParseMode.MARKDOWN,
                           reply_markup=keyboard)
        return
    ...
    if text_lower == "профиль":
        ...
        text_list += f"💬 Сообщений: {msg_count}\n"
