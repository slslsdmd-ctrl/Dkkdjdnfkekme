...
async def handle_knb(message: types.Message):
    ...
    if text in ["камень", "ножницы", "бумага"]:
        if (message.chat.id, message.from_user.id) not in knb_choices:
            await message.reply("ℹ️ Сначала начни игру командой **кнб** в ответ на сообщение противника!", parse_mode=ParseMode.MARKDOWN)
            return
        
        game_key = (message.chat.id, message.from_user.id)
        if game_key in knb_choices:
            await message.reply("✅ Твой выбор уже принят! Жди хода соперника...")
            return
...
async def handle_coinflip(message: types.Message):
    ...
    if text in ["орёл", "орел", "решка"]:
        if (message.chat.id, message.from_user.id) not in coin_choices:
            await message.reply("ℹ️ Сначала начни игру командой **орёл** или **решка** в ответ на сообщение противника!", parse_mode=ParseMode.MARKDOWN)
            return
        
        game_key = (message.chat.id, message.from_user.id)
        if game_key in coin_choices and coin_choices[game_key] is not None:
            await message.reply("✅ Твой выбор уже принят! Жди хода соперника...")
            return
